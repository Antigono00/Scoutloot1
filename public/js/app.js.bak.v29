...
async function handleAddWatch(event) {
  ...
  try {
    ...
    // V26: Send item_type, item_id, and minifig_image_url
    const watch = await apiCall('/watches', {
      ...
      body: JSON.stringify({
        ...
        condition: condition,
        minifig_image_url: selectedItemType === 'minifig' ? selectedMinifigImageUrl : null,
      }),
    });
...
// V26: Minifig selection state
let selectedMinifigId = null;
let selectedMinifigImageUrl = null;
...
function initAutocomplete() {
  ...
  input.addEventListener('input', (e) => {
    ...
    selectedSetNumber = null;
    // V26: Reset minifig selection on new input
    selectedMinifigId = null;
    selectedMinifigImageUrl = null;
    selectedItemType = 'set';
    ...
    autocompleteTimeout = setTimeout(() => {
      // V26: Use combined search instead of sets-only
...
// V26: Combined search for sets AND minifigs
  ...
  try {
    ...
    results.innerHTML = combined.map(item => {
      if (item._type === 'minifig') {
        const imgUrl = (item.set_img_url || '').replace(/'/g, "\\'");
        return `
          <div class="autocomplete-item" onclick="selectMinifig('${item.fig_num}', '${escapeHtml(item.name)}', '${imgUrl}')">
...
// Legacy: Keep searchSets for backward compatibility
async function searchSets(query) {
  return searchSetsAndMinifigs(query);
}
...
function selectSet(setNum, setName) {
  ...
  // V26: Reset minifig state when selecting a set
  selectedMinifigId = null;
  selectedMinifigImageUrl = null;
  selectedItemType = 'set';
  results.classList.remove('active');
...
// V26: New function to select a minifig
